...
class CrossSensoryNetwork(nn.Module):
    ...
    def forward(self, audio_input, tactile_input, visual_input):
        ...
        combined_features_out = (tactile_output + audio_output)/2.0

        # Concatenation for classification
        joint_representation = torch.cat([visual_output, combined_features_out], dim=1)
        joint_classification_output = self.joint_fc(joint_representation)

        return audio_output, tactile_output, visual_output, combined_features_out, joint_classification_output
